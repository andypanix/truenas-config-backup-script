...
By default, the script runs as a background daemon, waiting for the `SCHEDULED_TIME`. If you want to force an immediate backup (e.g., to test credentials or before performing a system upgrade), you can run the script with the `--now` flag.
...
If the container is not running (creates a temporary container that deletes itself after execution):
```bash
